...
export const Route = createFileRoute("/products/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    const paystackPage = getResoFlexPaystackPage(sku.slug);
    const image = paystackPage?.images?.[0];
    return {
      meta: [
      ...
      scripts: [
        { type: "application/ld+json", children: JSON.stringify({ "@context": "https://schema.org", "@type": "Product", name: sku.name, description: sku.description, sku: sku.slug, brand: { "@type": "Brand", name: "ResoFlex OS" }, ...(image ? { image: [image] } : {}), offers: { "@type": "Offer", priceCurrency: "NGN", price: sku.priceNGN, availability: "https://schema.org/InStock", url } }) },
        { type: "application/ld+json", children: JSON.stringify({ "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [ { "@type": "ListItem", position: 1, name: "Home", item: "https://store.resofit.fit/" }, { "@type": "ListItem", position: 2, name: "Tiers", item: "https://store.resofit.fit/tiers" }, { "@type": "ListItem", position: 3, name: sku.name, item: url } ] }) },
      ],
    };
  },
});
...
function NotFoundSKU() {
  return (
    <div className="mx-auto max-w-lg px-4 py-20 text-center">
      ...
      <p className="text-muted-foreground mt-2 text-sm">This slug isn't in the Sovereign catalog. Zero-dead-link policy — no fallback served.</p>
      <Link to="/tiers" className="inline-block mt-6"><GoldButton>See the Tier map</GoldButton></Link>
    </div>
  );
}
...
function ProductRoute() {
  ...
  const paystackPage = getResoFlexPaystackPage(sku.slug);
  const productImages = paystackPage?.images?.filter(Boolean) ?? [];
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 py-10">
      <Link to="/tiers" className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground hover:text-gold">← Tier map</Link>
      <div className="mt-4 grid md:grid-cols-2 gap-8">
        <div className="glass-panel rounded-xl overflow-hidden aspect-square relative bg-cover bg-center" style={heroImage ? { backgroundImage: `url(${heroImage})` } : undefined}>
          ...
          {heroImage && productImages.length > 1 && (
            <div className="absolute bottom-4 right-4 flex gap-1.5 max-w-[80%] overflow-x-auto rounded-lg bg-black/40 p-1.5 backdrop-blur-sm">
              {productImages.slice(0, 6).map((image, index) => <img key={`${image}-${index}`} src={image} alt={`${sku.name} image ${index + 1}`} className="size-12 rounded object-cover border border-white/20" loading={index === 0 ? "eager" : "lazy"} />)}
            </div>
          )}
        ...
        <div className="flex flex-col gap-4">
          <div><h1 className="font-display text-3xl sm:text-4xl font-bold">{sku.name}</h1><p className="text-muted-foreground mt-1">{sku.tagline}</p></div>
          <p className="text-sm text-muted-foreground leading-relaxed">{sku.description}</p>
          <div className="glass-panel rounded-lg p-5">
            <div className="flex items-baseline justify-between"><div className="font-display text-3xl font-bold text-gold">{sku.priceNGN === 0 ? "FREE" : `₦${total.toLocaleString()}`}</div>{sku.priceNGN > 0 && <div className="text-xs text-muted-foreground">₦{sku.priceNGN.toLocaleString()} × {qty}</div>}</div>
            {sku.priceNGN > 0 && <div className="mt-4 flex items-center gap-2"><span className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">Qty</span>{[1, 2, 3, 5, 10].map((n) => <button key={n} onClick={() => setQty(n)} className={`px-2.5 py-1 text-xs font-mono rounded border transition ${qty === n ? "bg-gold border-gold text-black font-bold" : "border-gold/20 text-muted-foreground hover:border-gold/50"}`}>{n}</button>)}</div>}
            <a href={checkoutHref} target="_blank" rel="noopener noreferrer" onClick={(e) => { const verified = verifyCheckoutUrl(checkoutHref); if (!verified.ok) { e.preventDefault(); track("checkout_guard_failure", { sku: sku.slug, reason: verified.reason, resolvedUrl: checkoutHref }); return; } track("checkout_guard_success", { sku: sku.slug, tier: sku.tier, qty, amount: total }); track("checkout_started", { sku: sku.slug, tier: sku.tier, qty, amount: total }); if (companion) track("recommendation_purchased", { sku: sku.slug, companion: companion.slug, surface: "product" }); }} className="mt-5 block">
              <GoldButton size="lg" className="w-full">{sku.priceNGN === 0 ? "Enter Funnel — Free" : "Deploy via Paystack"} <ExternalLink className="size-4" /></GoldButton>
            </a>
            <div className="mt-3 flex items-center justify-center gap-1.5 text-[10px] font-mono uppercase tracking-widest text-muted-foreground"><ShieldCheck className="size-3 text-gold" /> {sku.priceNGN === 0 ? "Funnel · attribution-tracked" : "Paystack · verified · encrypted"}</div>
          </div>
          <a href={`https://wa.me/${WHATSAPP_E164}?text=${encodeURIComponent(`ResoFlex — I want to deploy ${sku.name} (${sku.slug}). Tier ${sku.tier}.`)}`} target="_blank" rel="noopener noreferrer" onClick={() => track("whatsapp_click", { sku: sku.slug, source: "product_page" })} className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded border border-gold/30 text-gold hover:bg-gold/10 transition text-sm font-mono uppercase tracking-wider min-h-11"><MessageCircle className="size-4" /> Chat concierge on WhatsApp</a>
        </div>
      </div>
      ...
      <div className="mt-8 text-[10px] font-mono uppercase tracking-widest text-muted-foreground text-center">// Catalog-locked · {SOVEREIGN_CATALOG.length} SKUs · slug verified on load{paystackPage ? " · Paystack media attached" : ""}</div>
    </div>
  );
}
